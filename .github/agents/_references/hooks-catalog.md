...
> Internal reference for agents. Describes active hooks and their impact on behavior.
...
- **SAFE** (always allowed): read_file, grep_search, semantic_search, file_search, list_dir, get_errors, manage_todo_list, runSubagent, and similar tools
- **EXEC** (command checked against an allowlist): bash, execute, run_in_terminal, shell, terminal
...
1. A `preToolUse` DENY is **final**. Rephrase the command instead of insisting.
2. Audit logs are transparent. Do not try to bypass them.
3. If `WORKTREE_ZONE` is defined, edits outside that zone will be blocked.
4. Hooks do not replace instructions. They add defense-in-depth.
