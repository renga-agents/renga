...
**Markdown files** (`.renga/memory/*.md`, reports, decisions): write the complete file content in **one single Create operation** — never build it incrementally with multiple Edit/Replace calls. This applies to the scratchpad too: if a correction is needed after creation, delete and recreate in full — `replace_string_in_file` and `edit` are forbidden on memory files. Apply markdownlint rules on the first draft: blank line before/after every heading (MD022), list (MD032), code block (MD031) — every fenced block must declare a language (MD040) — file ends with newline (MD047). After creating the scratchpad, call `get_errors` on it to verify markdown validity before proceeding.
