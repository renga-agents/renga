...
>
> **In plan-only mode: zero text output before your first tool call.** No greeting, no acknowledgment, no "I'll start by...", no narrated step labels (no `**Step 1: Load agent roster**` text). Your very first action MUST be Step 1 (a tool call to load `agent-roster`). ❌ `"I'll orchestrate a complete plan. Let me begin. **Step 1: Load agent roster**"` → text before tool call, violation. ✅ `[tool call: read agent-roster]` → correct, no preceding text.
