...
- Creates `.copilot/memory/` for agent session memory
...
| `.copilot/memory/` | `renga init` | Local session data |
...
| --- | --- |
| `.copilot/memory/` | Steering memory — local data generated by agents over sessions, **specific to each project** |
| `.copilot/reports/` | Audit and session reports — local data generated automatically, **specific to each project** |
...
> [!IMPORTANT]
> The `.copilot/memory/` and `.copilot/reports/` folders may eventually be created automatically by agents when they run in your project. They contain local session data. Add them to your `.gitignore` if you do not want to commit them:
>
> ```gitignore
>
> .copilot/memory/
